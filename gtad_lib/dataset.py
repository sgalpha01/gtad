...
class VideoDataSet(data.Dataset):  # thumos
    def __init__(self, opt, subset="train", mode="train"):
        ...
        self.temporal_gap = 1.0 / self.temporal_scale  # 1/128
        self.subset = subset
        self.mode = mode
        ...
        if self.feature_path[-3:] == "200":
            self.feature_dirs = [
                self.feature_path + "/flow/csv",
                self.feature_path + "/rgb/csv",
            ]
        else:
    ...
    def _get_match_map(self):
        ...
        match_map = np.transpose(
            match_map, [1, 0, 2]
        )  # [0,1] [1,2] [2,3].....[99,100], 64 x 128 x 2
        match_map = np.reshape(
            match_map, [-1, 2]
        )  # [0,2] [1,3] [2,4].....[99,101]   # (duration x start) x 2
        ...
        self.anchor_xmax = [
            self.temporal_gap * (i + 0.5) for i in range(1, self.temporal_scale + 1)
        ]
    ...
    def _get_train_label(self, index):
        ...
        for j in range(len(gt_bbox)):
            ...
            tmp_gt_iou_map = np.reshape(
                tmp_gt_iou_map, [self.max_duration, self.num_videoframes]
            )
    ...
    def _get_data(self):
        ...
        saved_data_path = os.path.join(
            video_info_dir,
            "saved.%s.%s.nf%d.sf%d.num%d.%s.pkl"
            % (
                self.feat_dim,
                self.subset,
                self.num_videoframes,
                self.skip_videoframes,
                len(video_name_list),
                self.mode,
            ),
        ...
        for num_video, video_name in enumerate(video_name_list):
            print(
                "Getting video %d / %d" % (num_video, len(video_name_list)), flush=True
            )
            ...
            df_data = np.concatenate(
                [h5[:num_snippet, :] for h5 in feature_h5s], axis=1
            )
            ...
            elif num_snippet - windows_start[-1] - num_videoframes > int(
                num_videoframes / skip_videoframes
            ):
            ...
            for start in windows_start:
                ...
                if self.mode == "train":
                    tmp_anchor_xmins = tmp_snippets - skip_videoframes / 2.0
                    tmp_anchor_xmaxs = tmp_snippets + skip_videoframes / 2.0
                    tmp_gt_bbox = []
                    tmp_ioa_list = []
                    for idx in range(len(gt_xmins)):
                        tmp_ioa = ioa_with_anchors(
                            gt_xmins[idx],
                            gt_xmaxs[idx],
                            tmp_anchor_xmins[0],
                            tmp_anchor_xmaxs[-1],
        ...
        self.data = {"video_names": list_videos, "indices": list_indices}
        if self.mode == "train":
            self.data.update(
                {
