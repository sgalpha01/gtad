...
def train(data_loader, model, optimizer, epoch, bm_mask):
    ...
    for n_iter, (input_data, label_confidence, label_start, label_end) in enumerate(
        data_loader
    ):
        # forward pass
    ...
    print(
        "[Epoch {0:03d}]\tLoss {1:.2f} = {2:.2f} + {3:.2f} (train)".format(
...
def test(data_loader, model, epoch, bm_mask):
    ...
    print(
        "[Epoch {0:03d}]\tLoss {1:.2f} = {2:.2f} + {3:.2f} (validation)".format(
            epoch, total_am.avg(), subgraph_am.avg(), node_am.avg()
        )
    )
...
if __name__ == "__main__":
    ...
    optimizer = optim.Adam(
        model.parameters(), lr=opt["training_lr"], weight_decay=opt["weight_decay"]
    )
