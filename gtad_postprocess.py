...
def Soft_NMS(df, nms_threshold=1e-5, num_prop=200):
    ...
    while len(tscore) > 1 and len(rscore) < num_prop and max(tscore) > 0:
        ...
        for idx in range(0, len(tscore)):
            if idx != max_index:
                tmp_iou = IOU(
                    tstart[max_index], tend[max_index], tstart[idx], tend[idx]
                )
                if tmp_iou > 0:
...
def _gen_detection_video(
    video_name, video_cls, thu_label_id, opt, num_prop=200, topk=2
):

    files = [
        opt["output"] + "/results/" + f
        for f in os.listdir(opt["output"] + "/results/")
        if video_name in f
    ]
    if len(files) == 0:
    ...
    for snippet_file in files:
        ...
        snippet_df["score"] = (
            snippet_df.clr_score.values[:] * snippet_df.reg_socre.values[:]
        )
    ...
    for j in range(min(num_prop, len(df))):
        for k in range(topk):
            ...
            tmp_proposal["segment"] = [
                float(round(max(0, df.xmin.values[j]) / fps, 1)),
...
def gen_detection_multicore(opt):
    ...
    thu_label_id = (
        np.sort(thumos_test_anno.type_idx.unique())[1:] - 1
    )  # get thumos class id
    thu_video_id = np.array(
        [int(i[-4:]) - 1 for i in video_list]
    )  # -1 is to match python index

    # load video level classification
    cls_data = np.load("./data/uNet_test.npy")
    cls_data = cls_data[thu_video_id, :][
        :, thu_label_id
    ]  # order by video list, output 213x20
    ...
    result = {
        video: {
            "fps": thumos_gt.loc[thumos_gt["video-name"] == video]["frame-rate"].values[
                0
            ],
            "num_frames": thumos_gt.loc[thumos_gt["video-name"] == video][
                "video-frames"
            ].values[0],
        }
    ...
    detection = parallel(
        delayed(_gen_detection_video)(video_name, video_cls, thu_label_id, opt)
